...
let cardHTML = '';
if (n.coverUrl) {
  cardHTML += `<div class="card-img-wrapper"><img src="${escapeHtml(n.coverUrl)}" alt="${escapeHtml(n.title || '')}" loading="lazy" decoding="async" /></div>`;
}
cardHTML += `<div class="card-text-wrap">
...
if (totalPages > 1) {
  ...
  function showPage(page) {
    allCards.forEach((c, i) => {
      c.style.display = (i >= (page-1)*CARDS_PER_PAGE && i < page*CARDS_PER_PAGE) ? '' : 'none';
    });
    currentPage = page;
    renderPagination();
  ...
  function renderPagination() {
    let old = document.getElementById('gridPagination');
    if (old) old.remove();
    const pag = document.createElement('div');
    pag.id = 'gridPagination';
    pag.style.cssText = 'display:flex;justify-content:center;align-items:center;gap:8px;padding:30px 0 20px;';
    for (let p = 1; p <= totalPages; p++) {
      const btn = document.createElement('button');
      btn.textContent = p;
      btn.style.cssText = `padding:8px 14px;background:${p===currentPage?'rgba(201,168,76,0.2)':'transparent'};border:1px solid ${p===currentPage?'rgba(201,168,76,0.6)':'rgba(201,168,76,0.2)'};color:${p===currentPage?'var(--gold)':'var(--text-dim)'};font-family:Cinzel,serif;font-size:0.75rem;cursor:pointer;border-radius:4px;transition:all 0.2s;`;
...
function showConfirmToast(msg, onConfirm) {
  ...
  style.textContent = '@keyframes cfFadeIn{from{opacity:0}to{opacity:1}} @keyframes cfSlideUp{from{opacity:0;transform:translateY(22px) scale(0.96)}to{opacity:1;transform:translateY(0) scale(1)}} #__confirm_box__{animation:cfSlideUp 0.32s cubic-bezier(0.16,1,0.3,1);}';
  document.head.appendChild(style);
